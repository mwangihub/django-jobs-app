...
def validate_firstname_length(value):
    length = len(value)
    if length < 3:
        raise ValidationError("The length of the first name must be at least 3 characters in length")
    else:
        return value
...
def validate_lastname_length(value):
    length = len(value)
    if length < 3:
        raise ValidationError("The length of the last name must be at least 3 characters in length")
    else:
        return value
...
def validate_username_alphadigits(value):
    validmatch = re.match('^[A-Za-z0-9]+$', value)
    if not validmatch:
